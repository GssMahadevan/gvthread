...
fn accept_loop(listener: GvtListener, shared: Arc<ReactorShared>) {
...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--workers" | "-w" => {
                i += 1;
                num_workers = args.get(i).and_then(|s| s.parse().ok()).unwrap_or(4);
            }
            "--max-gvthreads" => {
                i += 1;
                max_gvthreads = args.get(i).and_then(|s| s.parse().ok()).unwrap_or(100_000);
            }
            "--sq" => {
                i += 1;
                sq_entries = args.get(i).and_then(|s| s.parse().ok()).unwrap_or(1024);
            }
    ...
    eprintln!("gvthread-httpd: model = one GVThread per connection + io_uring reactor");
    ...
    // ── 2. Start io_uring reactor ──
    let mut reactor = Reactor::start(ReactorConfig {
        sq_entries,
        max_slots: max_gvthreads,
    ...
    runtime.block_on(|| {
        let listener = GvtListener::bind(shared.clone(), port)
            .expect("failed to bind listener");

        // The accept loop itself is a GVThread
        let s = shared.clone();
        spawn(move |_token| {
            accept_loop(listener, s);
        });
